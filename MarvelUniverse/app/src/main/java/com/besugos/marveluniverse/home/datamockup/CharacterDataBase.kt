package com.besugos.marveluniverse.home.datamockup

import com.besugos.marveluniverse.home.model.*
import java.time.Instant
import java.util.*

class CharacterDataBase {

    companion object {
        val characters = listOf(CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                        "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                        "X-Facts",
                        "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                    "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            ), CharacterModel(1,
                "Abyss (Age of Apocalypse)",
                "Abyss is a Marvel Character",
                Date(), "http://gateway.marvel.com/v1/public/characters/1011266",
                UrlModel("detail",
                    "http://marvel.com/characters/2902/adam_destine?utm_campaign=apiRef&utm_source=001ac6c73378bbfff488a36141458af2"
                ), ImageModel("http://i.annihil.us/u/prod/marvel/i/mg/3/80/4c00358ec7548",
                    "jpg"
                ), StoryListModel(2,
                    2,
                    "http://gateway.marvel.com/v1/public/characters/1010903/stories",
                    listOf(StorySummaryModel("http://gateway.marvel.com/v1/public/stories/26280",
                        "X-Men: Alpha (1994) #1",
                        "cover"),
                        StorySummaryModel("http://gateway.marvel.com/v1/public/stories/38448",
                            "X-Facts",
                            "cover"))
                ), EventListModel(1,
                    1,
                    "http://gateway.marvel.com/v1/public/characters/1010903/events",
                    listOf(EventSummaryModel("http://gateway.marvel.com/v1/public/events/227",
                        "Age of Apocalypse"))
                )
            )
        )
    }

}